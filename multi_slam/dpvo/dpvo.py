...
class DPVO:
    def __init__(self, cfg, network, ht=480, wd=640, viz=False):
        ...
        self.targets_idxs = torch.full((self.N, self.M, 60), -1, dtype=torch.int, device="cuda")
        self.targets_lookup = torch.zeros(self.N, self.M, 60, 2, dtype=torch.float, device="cuda")
        self.weights_lookup = torch.zeros(self.N, self.M, 60, 2, dtype=torch.float, device="cuda")
    ...
    def keyframe(self):
        ...
        if m / 2 < self.cfg.KEYFRAME_THRESH:
            ...
            self.ii[self.ii > k] -= 1
            self.jj[self.jj > k] -= 1
            self.targets_idxs[self.targets_idxs > k] -= 1
            ...
            for i in range(k, self.n-1):
                ...
                self.targets_idxs[i] = self.targets_idxs[i+1]
                self.targets_lookup[i] = self.targets_lookup[i+1]
                self.weights_lookup[i] = self.weights_lookup[i+1]
