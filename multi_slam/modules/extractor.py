...
class MultiEncoder(nn.Module):
    def __init__(self, output_dims, norm_fn, dropout=0.0):
        super().__init__()
        self.norm_fn = norm_fn
        ...
        self.layer5 = self._make_layer(4*DIM, stride=1)
        self.up = nn.Upsample(scale_factor=2, mode='nearest')

        # output convolution
        self.output_conv = nn.ModuleList([nn.Conv2d(DIM, output_dims[0], kernel_size=1) if output_dims[0] else nn.Identity(),
                                          nn.Conv2d(2*DIM, output_dims[1], kernel_size=1) if output_dims[1] else nn.Identity(),
                                          nn.Conv2d(4*DIM, output_dims[2], kernel_size=1),
                                          nn.Conv2d(4*DIM, output_dims[2], kernel_size=1)]) # to remove
    ...
    def forward(self, x, padit=True):
        if padit:
            _, _, H, W = x.shape
            p = ceil(max(H, W) / 32) * 32
            x = F.pad(x, [0, p - W, 0, p - H], value=-1.0)
        x = self.conv1(x)
        x = self.norm1(x)
        x = self.relu1(x)

        x = self.layer1(x)
        x_2 = self.output_conv[0](x)
        x = self.layer2(x)
        x_4 = self.output_conv[1](x)
        x_8 = self.layer3(x)
        x_8b = self.layer5(x_8)

        x16 = self.layer4(x_8)
        x16b = self.up(x16.float()).to(x_8b)

        assert x_8b.shape == x16b.shape, (x_8b.shape, x16b.shape)
        final_x8 = self.output_conv[2](x_8b + x16b)

        if padit:
            return x_2[:,:,:H//2, :W//2], x_4[:,:,:H//4, :W//4], final_x8[:,:,:H//8, :W//8]
        return x_2, x_4, final_x8